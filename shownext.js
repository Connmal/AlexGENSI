...
// This part allows us to input the name they entered into the question line so it is customary for each alter.
    ...
    if (askedAbout > 0 && $('input[name=lk]:checked').length == 0 && !checked) {
      promptNonresponse();
      checked = true;
    } else {
      ...
      if (askedAbout > 0) {
        // Collect data before going on
        answers["q21.5_" + askedAbout.toLocaleString(undefined,{minimumIntegerDigits: 2})] = $('input[name=lk]:checked').val();
        ...
        document.getElementById("longknown").style.display = "block"; //Called from index.php this is calling the HTML for the age ticky boxes
      }
      ...
      if (askedAbout == numAlters) {
        ...
        nodes.forEach(function(d) {
          if (d.id > numAlters) {
            answers["q21.5_" + d.id.toLocaleString(undefined,{minimumIntegerDigits: 2})] = undefined;
          }
        });
        var d = new Date();
        answers["q21.5timeStamp"] = (d - startTime) / 1000;
        ...
        d3.select("svg").append("text")
          ...
          .attr("x", (currNode.x - 142 + 500 > bodyWidth) ? bodyWidth - 490 : Math.max(currNode.x - q_window_width / 2 - 100,10))
          .attr("dy", currNode.y - 340)
          .text("How long have you known " + nodes[askedAbout].name.toUpperCase() + " (in years) ?")
          .call(wrap, backdrop_width - 20);
    ...
    if (askedAbout > 0 && ($('input[name=rA]:checked').length == 0 || ($('input[name=rA]:checked').val() == 16 && $('input#rAtextInput').val() == "")) && !checked) {
      promptNonresponse();
      checked = true;
    } else {

      if (askedAbout == 0) {
        document.getElementById("question2_" + numAlters + "_window").style.display = "none";
        document.getElementById("backdrop2_" + numAlters).style.display = "none";
        document.getElementById("question2_" + numAlters).style.display = "none";

        document.getElementById("longknown").style.display = "none";
        ...
        for(i = 0;i < $('input[name=rA]:checked').length; i++) {
          ...
          if ($('input[name=rA]:checked')[i].value == 15) answers["q22_" + askedAbout.toLocaleString(undefined,{minimumIntegerDigits: 2})] += ":" + $('input#rAtextInput').val();
        }

        document.getElementById("question3_" + askedAbout + "_window").style.display = "none";
        document.getElementById("backdrop3_" + askedAbout).style.display = "none";
        document.getElementById("question3_" + askedAbout).style.display = "none";
      }
  ...
  if (($('input[name=age]:checked').val() == 1 && $('input#agetextInput').val() == "") || ($('input[name=age]:checked').length == 0) && !checked) {
    ...
    answers.q36AtimeStamp = (d - startTime) / 1000;
      ...
      if (($('input[name=gender]:checked').length == 0)  && !checked && !skipped) {
        ...
        var ex = document.getElementById("relation_input");
         ex.style.left = string_l + "px";
         ex.style.top = string_t;
         ex.style.display = "block";
       ...
       if (($('input[name=relation]:checked').val() == 7 && $('input#relationtextInput').val() == "") || ($('input[name=relation]:checked').length == 0) && !checked) {
         promptNonresponse();
         checked = true;
       } else {
         // Collect data before going on
         answers.q57_8D = $('input[name=relation]:checked').val();
         if ($('input[name=relation]:checked').val() == 7) answers.q57_8_D += ":" + $('input#relationtextInput').val();
         var d = new Date();
         answers.q57_8DtimeStamp = (d - startTime) / 1000;
           // Collect data before going on

      document.getElementById("relation_input").style.display = "none";

     var ex = document.getElementById("covid_input");
     ex.style.left = string_l + "px";
     ex.style.top = string_t;
     ex.style.display = "block";
   ...
   if (($('input[name=covid]:checked').val() == 2 && $('input#covidtextInput').val() == "") || ($('input[name=covid]:checked').length == 0) && !checked) {
     promptNonresponse();
     checked = true;
   } else {
     // Collect data before going on
     answers.q57_8D = $('input[name=covid]:checked').val();
     if ($('input[name=covid]:checked').val() == 2) answers.q57_8_D += ":" + $('input#covidtextInput').val();
     var d = new Date();
     answers.q57_8DtimeStamp = (d - startTime) / 1000;
       // Collect data before going on

     document.getElementById("covid_input").style.display = "none";
    ...
    if (($('input[name=ha13]:checked').length == 0 || $('input[name=ha14]:checked').length == 0) && !checked && !skipped) {
      ...
      var ex = document.getElementById("WBS1");
      ex.style.left = string_l + "px";
      ex.style.top = string_t;
      ex.style.display = "block";
    ...
    if (($('input[name=wb1]:checked').length == 0 || $('input[name=wb2]:checked').length == 0) && !checked && !skipped) {
      promptNonresponse();
      checked = true;
    } else {
      // Collect data before going on
      if(!skipped) {
        answers.q47a = $('input[name=wb1]:checked').val();
        answers.q47b = $('input[name=wb2]:checked').val();
        var d = new Date();
...
// Instagram Questions signified by the 'in(number)' Called from the index.php 2 questions per page
      ...
      document.getElementById("WBS1").style.display = "none";
      var ex = document.getElementById("WBS3");
      ex.style.left = string_l + "px";
      ex.style.top = string_t;
      ex.style.display = "block";
    ...
    if (($('input[name=wb3]:checked').length == 0 || $('input[name=wb4]:checked').length == 0) && !checked && !skipped) {
      promptNonresponse();
      checked = true;
    } else {
      // Collect data before going on
      if(!skipped) {
        answers.q48a = $('input[name=wb3]:checked').val();
        answers.q48b = $('input[name=wb4]:checked').val();
        var d = new Date();
      ...
      document.getElementById("WBS3").style.display = "none";
      var ex = document.getElementById("WBS5");
      ex.style.left = string_l + "px";
      ex.style.top = string_t;
      ex.style.display = "block";
    ...
    if (($('input[name=wb5]:checked').length == 0 || $('input[name=wb6]:checked').length == 0) && !checked && !skipped) {
      promptNonresponse();
      checked = true;
    } else {
      // Collect data before going on
      if(!skipped) {
        answers.q49a = $('input[name=wb5]:checked').val();
        answers.q49b = $('input[name=wb6]:checked').val();
        var d = new Date();
      ...
      document.getElementById("WBS5").style.display = "none";
      var ex = document.getElementById("WBS7");
      ex.style.left = string_l + "px";
      ex.style.top = string_t;
      ex.style.display = "block";
    ...
    if (($('input[name=wb7]:checked').length == 0 || $('input[name=wb8]:checked').length == 0) && !checked && !skipped) {
      promptNonresponse();
      checked = true;
    } else {
      // Collect data before going on
      if(!skipped) {
        answers.q49a1 = $('input[name=wb7]:checked').val();
        answers.q49b1 = $('input[name=wb8]:checked').val();
        var d = new Date();
        answers.q491timeStamp = (d - startTime) / 1000;
      } else {
        answers.q49a1 = "skipped";
        answers.q49b1 = "skipped";
        var d = new Date();
        answers.q491timeStamp = (d - startTime) / 1000;
      }
      // Collect data before going on
      ...
      document.getElementById("WBS7").style.display = "none";
      var ex = document.getElementById("WBS9");
      ex.style.left = string_l + "px";
      ex.style.top = string_t;
      ex.style.display = "block";
    ...
    if (($('input[name=wb9]:checked').length == 0 || $('input[name=wb10]:checked').length == 0) && !checked && !skipped) {
      promptNonresponse();
      checked = true;
    } else {
      // Collect data before going on
      if(!skipped) {
        answers.q49a2 = $('input[name=wb9]:checked').val();
        answers.q49b2 = $('input[name=wb10]:checked').val();
        var d = new Date();
        answers.q492timeStamp = (d - startTime) / 1000;
      } else {
        answers.q49a2 = "skipped";
        answers.q49b2 = "skipped";
        var d = new Date();
        answers.q492timeStamp = (d - startTime) / 1000;
      }
      // Collect data before going on
      ...
      document.getElementById("WBS9").style.display = "none";
      var ex = document.getElementById("WBS11");
      ex.style.left = string_l + "px";
      ex.style.top = string_t;
      ex.style.display = "block";
    ...
    if (($('input[name=wb11]:checked').length == 0 || $('input[name=wb12]:checked').length == 0) && !checked && !skipped) {
      promptNonresponse();
      checked = true;
    } else {
      // Collect data before going on
      if(!skipped) {
        answers.q49a3 = $('input[name=wb11]:checked').val();
        answers.q49b3 = $('input[name=wb12]:checked').val();
        var d = new Date();
        answers.q493timeStamp = (d - startTime) / 1000;
      } else {
        answers.q49a3 = "skipped";
        answers.q49b3 = "skipped";
        var d = new Date();
        answers.q493timeStamp = (d - startTime) / 1000;
      }
      // Collect data before going on
      ...
      document.getElementById("WBS11").style.display = "none";
    ...
    if (($('input[name=br]:checked').val() == 5 && $('input#brtextInput').val() == "") || ($('input[name=br]:checked').length == 0) && !checked) {
      ...
      var ex = document.getElementById("finance_input");
      ex.style.left = string_l + "px";
      ex.style.top = string_t;
      ex.style.display = "block";
    ...
    if (($('input[name=finance]:checked').length == 0)  && !checked && !skipped) {
      promptNonresponse();
      checked = true;
    } else {
      // Collect data before going on
      if(!skipped) {
        answers.q58 = $('input[name=finance]:checked').val();
        var d = new Date();
        answers.q58timeStamp = (d - startTime) / 1000;
      } else {
        answers.q58 = "skipped";
      }
